...
class PCA(object):
    ...
    def fit(self, X, method='svd'):
        ...
        if method == "svd":
            ...
            self.W_pca, vals = self._eigen_decomp(X_centered)
        # END YOUR CODE
    ...
    def _eigen_decomp(self, X):
        ...
        args = np.argsort(-e_vals)
        e_vals = e_vals[args]
        e_vecs = (e_vecs.T[args]).T
        # END YOUR CODE
...
class LDA(object):
    ...
    def _within_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            ...
            X_i = X[y == i]
            scatter_within += np.cov(X_i - np.mean(X_i, axis=0), rowvar=False)
            # END YOUR CODE
    ...
    def _between_class_scatter(self, X, y):
        ...
        for i in np.unique(y):
            # YOUR CODE HERE
            #a[y == i] = np.mean(X[y == i], axis=0)
            mu_i = np.mean(X[y == i], axis=0).reshape(-1, 1) - mu.reshape(-1, 1)
            scatter_between += X[y == i].shape[0] * mu_i.dot(mu_i.T)
        #scatter_between = np.cov(a - mu, rowvar=False)
            # END YOUR CODE
