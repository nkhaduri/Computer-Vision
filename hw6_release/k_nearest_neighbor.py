...
def compute_distances(X1, X2):
    ...
    #Y1 = np.repeat(X1.reshape(1, M, D), N, axis=0)
    #Y2 = np.repeat(X2.reshape(1, N, D), M, axis=0).transpose(1, 0, 2)
    #dist = Y2 - Y1
    #dists = dists * dists
    
    v2 = np.repeat(np.sum(X1 * X1, axis=1).reshape(-1, 1), N, axis=1)
    u2 = np.repeat(np.sum(X2 * X2, axis=1).reshape(1, -1), M, axis=0)
    uv = X1.dot(X2.T)
    dists = np.sqrt(v2 + u2 - 2 * uv)
    # END YOUR CODE
...
def predict_labels(dists, y_train, k=1):
    ...
    for i in range(num_test):
        ...
        # YOUR CODE HERE
        y_pred[i] = np.argmax(np.bincount(y_train[np.argsort(dists[i])[:k]]))
        # END YOUR CODE
...
def split_folds(X_train, y_train, num_folds):
    ...
    X_vals = np.asarray(np.array_split(X_train, num_folds))
    y_vals = np.asarray(np.array_split(y_train, num_folds))
    for i in range(num_folds):
        X_trains[i] = np.delete(X_vals, i, axis=0).reshape(training_size, X_train.shape[1])
        y_trains[i] = np.delete(y_vals, i, axis=0).reshape(-1, )
    # END YOUR CODE
