...
def energy_function(image):
    ...
    grads = np.gradient(img_gray)
    out = abs(grads[0]) + abs(grads[1])
    ### END YOUR CODE
...
def compute_cost(image, energy, axis=1):
    ...
    for i in range(1, H):
        tmp[1] = cost[i-1]
        tmp[0] = np.delete(np.insert(cost[i-1], 0, 100000), [W])
        tmp[2] = np.delete(np.insert(cost[i-1], W, 100000), [0])
        paths[i] = np.argmin(tmp, axis=0) - 1
        cost[i] = energy[i] + np.min(tmp, axis=0) 
    ### END YOUR CODE
...
def duplicate_seam(image, seam):
    ...
    for j in range(C):
        for i in range(H):
            out[i,:,j] = np.insert(image[i,:,j], seam[i], image[i, seam[i], j])
    ### END YOUR CODE
...
def enlarge(image, size, axis=1, efunc=energy_function, cfunc=compute_cost):
    ...
    seams_indexes = np.zeros((size - W, H))
    for i in range(size - W):
        index = np.where(seams == i + 1)
        seams_indexes[i] = np.copy(index[1])
    seams_indexes = seams_indexes.astype(int)
    for i in range(size - W):
        seam = seams_indexes[i]
        out = duplicate_seam(out, seam)
        seams_indexes[seams_indexes > seam] += 1
    ### END YOUR CODE
...
def compute_forward_cost(image, energy):
    ...
    ### YOUR CODE HERE
    tmp = np.zeros((3, W))
    img_row_shifted = np.zeros((W, ))
    img_row_tmp = np.zeros((W, ))
    img_shifted_left = np.zeros((W, ))
    img_shifted_right = np.zeros((W, ))
    for i in range(1, H):
        tmp[1] = cost[i-1]
        tmp[0] = np.delete(np.insert(cost[i-1], 0, 100000), [W])
        tmp[2] = np.delete(np.insert(cost[i-1], W, 100000), [0])
        img_shifted_right = np.delete(np.insert(image[i], 0, image[i-1][0]), [W])
        tmp[0] += np.abs(img_shifted_right - image[i-1])
        img_shifted_left = np.delete(np.insert(image[i], W, image[i-1][W-1]), [0])
        tmp[2] += np.abs(img_shifted_left - image[i-1])
        
        paths[i] = np.argmin(tmp, axis=0) - 1
        
        img_row_shifted = np.delete(np.insert(image[i], 0, image[i][1]), [W])
        img_row_tmp = np.delete(np.insert(image[i], W, image[i][W-2]), [0])
        cost[i] = energy[i] + np.min(tmp, axis=0) + np.abs(img_row_shifted - img_row_tmp)
    ### END YOUR CODE
