...
def sliding_window(image, base_score, stepSize, windowSize, pixel_per_cell=8):
    ...
    for i in range(0, H, stepSize):
        for j in range(0, W, stepSize):
            score = feature.hog(pad_image[i:i + winH, j:j+winW], pixels_per_cell=(pixel_per_cell, 
                                                                                      pixel_per_cell)).dot(base_score.T)
            if score >= max_score:
                max_score = score
                maxr = i - winH // 2
                maxc = j - winW // 2
...
def pyramid(image, scale=0.9, minSize=(200, 100)):
    ...
    while True:
        current_scale *= scale
        img = rescale(image, current_scale)
...
def pyramid_score(image,base_score, shape, stepSize=20, scale = 0.9, pixel_per_cell = 8):
    ...
    for x in images:
        img = x[1]
        scale = x[0]
        max_sc, max_r, max_c, response_map = sliding_window(img, base_score, stepSize, shape, pixel_per_cell)
        if max_sc > max_score:
            max_score = max_sc
            max_scale = scale
            maxr = max_r
...
def shift_heatmap(heatmap, mu):
    ...
    '''
    ### YOUR CODE HERE
    #new_heatmap = heatmap - np.mean(heatmap)
    #new_heatmap /= np.std(heatmap)
    new_heatmap = interpolation.shift(heatmap / np.max(heatmap), mu)
    ### END YOUR CODE
    return new_heatmap
